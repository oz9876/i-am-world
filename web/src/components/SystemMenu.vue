...
import EncyclopediaPanel from './game/panels/system/EncyclopediaPanel.vue'
...
const props = defineProps<{
  visible: boolean
  defaultTab?: 'save' | 'load' | 'create' | 'delete' | 'llm' | 'start' | 'settings' | 'about' | 'encyclopedia' | 'other'
...
const activeTab = ref<'save' | 'load' | 'create' | 'delete' | 'llm' | 'start' | 'settings' | 'about' | 'encyclopedia' | 'other'>(props.defaultTab || 'load')
...
<template>
  <div v-if="visible" class="system-menu-overlay">
    <div class="system-menu">
      ...
      <div class="menu-tabs">
        <button :class="{ active: activeTab === 'start' }" @click="switchTab('start')" v-sound:select>
          {{ t('ui.start_game') }}
        </button>
        <button :class="{ active: activeTab === 'load' }" @click="switchTab('load')" v-sound:select>
          {{ t('ui.load_game') }}
        </button>
        <button :class="{ active: activeTab === 'encyclopedia' }" @click="switchTab('encyclopedia')" v-sound:select>
          {{ t('ui.encyclopedia', '图鉴') }}
        </button>
        <button :class="{ active: activeTab === 'save' }" @click="switchTab('save')" :disabled="!gameInitialized"
          v-sound:select>
          {{ t('ui.save_game') }}
        </button>
        <button :class="{ active: activeTab === 'create' }" @click="switchTab('create')" :disabled="!gameInitialized"
          v-sound:select>
          {{ t('ui.create_character') }}
        </button>
        <button :class="{ active: activeTab === 'delete' }" @click="switchTab('delete')" :disabled="!gameInitialized"
          v-sound:select>
          {{ t('ui.delete_character') }}
        </button>
        <button :class="{ active: activeTab === 'llm' }" @click="switchTab('llm')" v-sound:select>
          {{ t('ui.llm_settings') }}
        </button>
        <button :class="{ active: activeTab === 'settings' }" @click="switchTab('settings')" v-sound:select>
          {{ t('ui.settings') }}
        </button>
        <button :class="{ active: activeTab === 'about' }" @click="switchTab('about')" v-sound:select>
          {{ t('ui.about') }}
        </button>
        <button :class="{ active: activeTab === 'other' }" @click="switchTab('other')" v-sound:select>
          {{ t('ui.other') }}
        </button>
      </div>

      <div class="menu-content">
        <GameStartPanel v-if="activeTab === 'start'" :readonly="gameInitialized" />

        <EncyclopediaPanel v-else-if="activeTab === 'encyclopedia'" />

        <SaveLoadPanel v-else-if="activeTab === 'save' || activeTab === 'load'"
          :mode="activeTab === 'save' ? 'save' : 'load'" @close="emit('close')" />

        <CreateAvatarPanel v-else-if="activeTab === 'create'" @created="switchTab('create')" />

        <DeleteAvatarPanel v-else-if="activeTab === 'delete'" />

        <LLMConfigPanel v-else-if="activeTab === 'llm'" @config-saved="emit('llm-ready')" />
        ...
        <div v-else-if="activeTab === 'settings'" class="settings-panel-container">
          ...
          <div class="settings-form">
              ...
              <div class="sound-controls">
                ...
                <div class="volume-row">
                  ...
                  <div class="slider-container">
                    <n-slider v-model:value="settingStore.bgmVolume" :min="0" :max="1" :step="0.05" :tooltip="false"
                ...
                <div class="volume-row">
                  ...
                  <div class="slider-container">
                    <n-slider v-model:value="settingStore.sfxVolume" :min="0" :max="1" :step="0.05" :tooltip="false"
            ...
            <div class="setting-item">
              ...
              <n-select v-model:value="settingStore.locale" :options="languageOptions"
                @update:value="settingStore.setLocale" style="width: 200px" />
            </div>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'about'" class="other-panel-container">
          ...
          <div class="other-actions">
            <button class="custom-action-btn"
              @click="openLink('https://github.com/4thfever/cultivation-world-simulator')" v-sound>
            ...
            <button class="custom-action-btn"
              @click="openLink('https://github.com/4thfever/cultivation-world-simulator/blob/main/CONTRIBUTORS.md')"
              v-sound>
              <div class="btn-content">
                <div class="btn-icon">👥</div>
                <div class="btn-text-group">
